...
void* handle_client_thread(int sockfd){ // arg is an int pointer that contains the socked_id of our client.
    char buffer[MAXDATASIZE]; //to receive the messages of the client.
    while(true){ //while the connection exists:
        memset(buffer,0,sizeof(buffer));
        int nbytes = recv(sockfd, buffer , sizeof(buffer),0); // waiting and blocking .
...
int main(void)
{
    ...
    while (true) { // an endless loop , the sever always open for new clients.
        ...
        int newfd = accept(listener, (struct sockaddr*)&remoteaddr, &addrlen); // new fd that waits for a new connection from a client
        ...
        pthread_t tid = startProactor(newfd, handle_client_thread);
        if (tid == 0) {
            fprintf(stderr, "Failed to start proactor for socket %d\n", newfd);
            close(newfd);
        }
    }
